...
# Global vars, will clean up later 
DIFFICULTYS = [0, 1, 2]
METRICS = ['precision', 'recall', 'TP', 'FP', 'FN', 'num_gts']
...
def clean_data(gt_anno, dt_anno, current_class, difficulty,all_classes):
    CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck'] if all_classes is None else all_classes
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty,all_classes=None):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty,all_classes)
...
def eval_class(gt_annos,
    ...
    TP = np.zeros([num_class, num_difficulty, num_minoverlap])
    FP = np.zeros([num_class, num_difficulty, num_minoverlap])
    FN = np.zeros([num_class, num_difficulty, num_minoverlap])
    num_gts = np.zeros([num_class, num_difficulty])

    class_names = [class_to_name[c].lower() for c in current_classes]
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty,all_classes=class_names)
            (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets,
             dontcares, total_dc_num, total_num_valid_gt) = rets
            
            num_gts[m,l] = total_num_valid_gt
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for j, num_part in enumerate(split_parts):
                    ...
                    if pr.shape[0] == 0:
                        print("??")
                TP[m, l, k] =  pr[-1, 0] # True positives
                FP[m, l, k] =  pr[-1, 1] # False positives
                FN[m, l, k] =  pr[-1, 2] # False negatives
...
def do_eval(gt_annos,
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 0,
                     min_overlaps, compute_aos,class_to_name=class_to_name)
    ...
    if PR_detail_dict is not None:
        PR_detail_dict['bbox'] = {}
        for key in ret:
            PR_detail_dict['bbox'][key] = ret[key]
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1,
                     min_overlaps,class_to_name=class_to_name)
    ...
    if PR_detail_dict is not None:
        PR_detail_dict['bev'] = {}
        for key in ret:
            PR_detail_dict['bev'][key] = ret[key]
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, eval_overlap,PR_detail_dict=None):
    ...
    min_overlaps = np.transpose(np.array(eval_overlap),(1,2,0))
    # assert np.array_equal(min_overlaps,np.transpose(np.array(eval_overlap),(1,2,0)))
    class_to_name = {i:current_classes[i] for i in range(len(current_classes))}
    # class_to_name = {
    #     0: 'Car',
    #     1: 'Pedestrian',
    #     2: 'Cyclist',
    ...
    name_to_metric = {v: n for n, v in metric_to_name.items()}
    ...
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict,class_to_name=class_to_name)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        curname = class_to_name[curcls]
        eval_metrics[curname] = {
            'bbox': {},
            'bev': {},
            '3d': {}
        } 
        headers = [f"Difficulty", "0", "1", "2"]
        r11_table = []
        r40_table = []
        
        for i in range(min_overlaps.shape[0]):
            r11_table.append([f"bbox (IoU={min_overlaps[i, 0, j]:.2f})",
                        f"{mAPbbox[j, 0, i]:.4f}",
                        f"{mAPbbox[j, 1, i]:.4f}",
                        f"{mAPbbox[j, 2, i]:.4f}"])
            r11_table.append([f"bev  (IoU={min_overlaps[i, 1, j]:.2f})",
                        f"{mAPbev[j, 0, i]:.4f}",
                        f"{mAPbev[j, 1, i]:.4f}",
                        f"{mAPbev[j, 2, i]:.4f}"])
            r11_table.append([f"3d   (IoU={min_overlaps[i, 2, j]:.2f})",
                        f"{mAP3d[j, 0, i]:.4f}",
                        f"{mAP3d[j, 1, i]:.4f}",
                        f"{mAP3d[j, 2, i]:.4f}"])
            if compute_aos:
                r11_table.append([f"aos",
                            f"{mAPaos[j, 0, i]:.2f}",
                            f"{mAPaos[j, 1, i]:.2f}",
                            f"{mAPaos[j, 2, i]:.2f}"])

                if i != min_overlaps.shape[0] - 1:
                    r11_table.append(["-"*5]*4)
                # if i == 0:
            ...
            for m_idx, metric_name in metric_to_name.items():
                eval_metrics[curname][metric_name][min_overlaps[i,:,j][m_idx]] = {
                    metric: {
                        idx: PR_detail_dict[metric_name][metric][j, idx, i]
                        if metric != 'num_gts' else PR_detail_dict[metric_name][metric][j, idx]
                            for idx in DIFFICULTYS
                    }
                    for metric in METRICS
                }

        result += f"{class_to_name[curcls]} (R11)\n"
        result += tabulate(r11_table, headers, tablefmt="pretty")
        result += "\n"
        result += f"{class_to_name[curcls]} (R40)\n"
        result += tabulate(r40_table, headers, tablefmt="pretty")
        result += "\n"
